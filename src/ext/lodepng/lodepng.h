...
/*pass -DLODEPNG_NO_COMPILE_ZLIB to the compiler to disable this, or comment out LODEPNG_COMPILE_ZLIB
 * below*/
...
/*pass -DLODEPNG_NO_COMPILE_DISK to the compiler to disable this, or comment out LODEPNG_COMPILE_DISK
 * below*/
...
The default built-in CRC code comes with 8KB of lookup tables, so for memory constrained environment you
may want it disabled and provide a much smaller implementation externally as said above. You can find
such an example implementation in a comment in the lodepng.c(pp) file in the 'else' case of the
...
struct LodePNGDecompressSettings {
  ...
  unsigned ignore_adler32; /*if 1, continue and don't give an error message if the Adler32 checksum is
                              corrupted*/
...
struct LodePNGCompressSettings /*deflate = compress*/ {
  ...
  unsigned btype;      /*the block type for LZ (0, 1, 2 or 3, see zlib standard). Should be 2 for proper
                          compression.*/
  unsigned use_lz77;   /*whether or not to use LZ77. Should be 1 for proper compression.*/
  unsigned windowsize; /*must be a power of two <= 32768. higher compresses more but is slower. Default
                          value: 2048.*/
  unsigned
    minmatch; /*minimum lz77 length. 3 is normally best, 6 can be better for some PNGs. Default: 0*/
  unsigned
...
typedef struct LodePNGColorMode {
  ...
  */
  unsigned char
    *palette; /*palette in RGBARGBA... order. Must be either 0, or when allocated must have 1024 bytes*/
...
Returns true if the image can have translucent or invisible pixels (it still be opaque if it doesn't use
such pixels). Returns false if the image can only have opaque pixels. In detail, it returns true only if
it's a color type with alpha, or has a palette with non-opaque values, or if "key_defined" is true.
*/
...
typedef struct LodePNGInfo {
  ...
  */
  ...
  The exif data is in exif-encoded form but without JPEG markers, starting with the 'II' or 'MM' marker
  that indicates endianness. It's up to an exif handling library to encode/decode its information.
  */
  ...
  /*phys chunk (pHYs)*/
  unsigned phys_defined; /*if 0, there is no pHYs chunk and the values below are undefined, if 1 else
                            there is one*/
  ...
  LodePNG does not apply any color conversions on pixels in the encoder or decoder and does not interpret
  these color profile values. It merely passes on the information. If you wish to use color profiles and
  convert colors, a separate color management library should be used. There is also a limited library for
  this in lodepng_util.h.

  There are 4 types of (sets of) chunks providing color information. If multiple are present, each will
  be decoded by LodePNG, but only one should be handled by the user, with the following order of priority
  depending on what the user supports: 1: cICP: Coding-independent code points (CICP) 2: iCCP: ICC
  profile 3: sRGB: indicates the image is in the sRGB color profile 4: gAMA and cHRM: indicates a gamma
  and chromaticity value to define the color profile
  */
  ...
  */
  ...
  unsigned srgb_intent;  /* Rendering intent: 0=perceptual, 1=rel. colorimetric, 2=saturation, 3=abs.
                            colorimetric */
  ...
  Contains ICC profile, which can use any version of the ICC.1 specification by the International Color
  Consortium. See its specification for more details. LodePNG does not parse or use the ICC profile
  (except its color space header field for "RGB" or "GRAY", see below), a separate library to handle the
  ICC data format is needed to use it for color management and conversions.

  For encoding, if iCCP is present, the PNG specification recommends to also add gAMA and cHRM chunks
  that approximate the ICC profile, for compatibility with applications that don't use the ICC chunk.
  This is not required, and it's up to the user to compute approximate values and set then in the
  appropriate gama_ and chrm_ fields, LodePNG does not do this automatically since it does not interpret
  the ICC profile.

  For encoding, the ICC profile is required by the PNG specification to be an "RGB" profile for non-gray
  PNG color types (types 2, 3 and 6) and a "GRAY" profile for gray PNG color types (types 1 and 4). If
  you disable auto_convert, you must ensure the ICC profile type matches your requested color type, else
  the encoder gives an error. If auto_convert is enabled (the default), and the ICC profile is not a
  correct match for the pixel data, this will result in an encoder error if the pixel data has non-gray
  pixels for a GRAY profile, or a silent less-optimal compression of the pixel data if the pixels could
  be encoded as grayscale but the ICC profile is RGB.

  To avoid this do not set an ICC profile in the image unless there is a good reason for it, and when
  doing so make sure you compute it carefully to avoid the above problems.
  */
  ...
  */
  ...
  If defined, these values give the bit depth of the original data. Since PNG only stores 1, 2, 4, 8 or
  16-bit per channel data, the significant bits value can be used to indicate the original encoded data
  has another sample depth, such as 10 or 12.
...
/*destructs the LodePNGInfo and brings it to invalid state, requiring lodepng_info_init again before
 * reusing it*/
...
typedef struct LodePNGDecoderSettings {
  ...
  unsigned
    ignore_end; /*ignore issues at end of file if possible (missing IEND chunk, too large chunk, ...)*/
  /* TODO: make a system involving warnings with levels and a strict mode instead. Other potentially
     recoverable errors: srgb rendering intent value, size of content of ancillary chunks, more than 79
     characters for some strings, placement/combination rules for ancillary chunks, crc of unknown
     chunks, allowed characters in string keys, invalid characters in chunk types names, etc... */
  ...
  /* maximum size for decompressed text chunks. If a text chunk's text is larger than this, an error is
  returned, unless reading text chunks is disabled or this limit is set higher or disabled. Set to 0 to
  allow any size. By default it is a value that prevents unreasonably large strings from hogging memory.
*/
  size_t max_text_size;

  /* maximum size for compressed ICC chunks. If the ICC profile is larger than this, an error will be
  returned. Set to 0 to allow any size. By default this is a value that prevents ICC profiles that would
  be much larger than any legitimate profile could be to hog memory. */
  size_t max_icc_size;
#endif /*LODEPNG_COMPILE_ANCILLARY_CHUNKS*/
...
typedef enum LodePNGFilterStrategy {
  ...
  LFS_THREE = 3,
  LFS_FOUR = 4,
  /*Use the filter out of the 5 above types that gives minimum sum, by trying each one. This is the
  adaptive filtering suggested heuristic in the PNG standard chapter 'Filter selection'.*/
  LFS_MINSUM,
...
typedef struct LodePNGColorStats {
  ...
  unsigned short
    key_r; /*key values, always as 16-bit, in 8-bit case the byte is duplicated, e.g. 65535 means 255*/
  unsigned short key_g;
  unsigned short key_b;
  unsigned alpha; /*image is not opaque and alpha channel or alpha palette required*/
  unsigned
    numcolors; /*amount of colors, up to 257. Not valid if bits == 16 or allow_palette is disabled.*/
  unsigned char palette[1024]; /*Remembers up to the first 256 RGBA colors, in no particular order, only
                                  valid when numcolors is valid*/
  unsigned bits; /*bits per channel (not for palette). 1,2 or 4 for grayscale only. 16 if 16-bit per
                    channel required.*/
  size_t numpixels;

  /*user settings for computing/using the stats*/
  unsigned allow_palette; /*default 1. if 0, disallow choosing palette colortype in auto_choose_color,
                             and don't count numcolors*/
  unsigned
    allow_greyscale; /*default 1. if 0, choose RGB or RGBA even if the image only has gray colors*/
} LodePNGColorStats;
...
unsigned lodepng_compute_color_stats(LodePNGColorStats *stats, const unsigned char *image, unsigned w,
...
/*destructs the LodePNGState and brings it to invalid state, requiring lodepng_info_init again before
 * reusing it*/
...
terminator in text, ... [ ] error messages with line numbers (and version) [ ] errors in state instead of
as return code? [ ] new errors/warnings like suspiciously big decompressed ztxt or iccp chunk [ ] let the
C++ wrapper catch exceptions coming from the standard library and return LodePNG error codes [ ] allow
user to provide custom color conversion functions, e.g. for premultiplied alpha, padding bits or not, ...
...
*) colorimetric color profile conversions: currently experimentally available in lodepng_util.cpp only,
   plus alternatively ability to pass on chroma/gamma/ICC profile information to other color management
system.
...
-if a struct has a corresponding cleanup function, call it before the struct disappears to avoid memory
leaks -if a struct has a corresponding copy function, use the copy function instead of "=". The
...
*) LodePNGColorMode info_raw: here you can say what color mode of the raw image (the output) you want to
get
