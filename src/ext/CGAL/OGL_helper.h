...
public:
  ...
  operator double_ptr() const { return const_cast<Double_triple&>(*this).coords_; }
...
public:
  ...
  void push_back_vertex(double x, double y, double z) { coords_.push_back(Double_triple(x, y, z)); }
  ...
  void push_back_vertex(const Double_point& p) { push_back_vertex(p.x(), p.y(), p.z()); }
  ...
  void debug(std::ostream& os = std::cerr) const
  {
    ...
    for (unsigned i = 0; i < number_of_facet_cycles(); ++i) {
      ...
      for (cit = facet_cycle_begin(i); cit != facet_cycle_end(i); ++cit) os << *cit;
      os << std::endl;
    }
  }
...
inline void CGAL_GLU_TESS_CALLBACK vertexCallback(GLvoid *vertex, GLvoid *user)
{
                                                   ...
                                                   GLvoid **dataOut)
{
  ...
  if (dataOut) {
    ...
    *dataOut = n;
    ...
    for (std::list<GLdouble *>::const_iterator i = pcache.begin(); i != pcache.end(); i++) delete[] *i;
    pcache.clear();
  }
}
...
public:
  ...
  void push_back(const Double_point& p, bool m) { vertices_.push_back(DPoint(p, m)); }
  void push_back(const Double_segment& s, bool m) { edges_.push_back(DSegment(s, m)); }
  void push_back(const DFacet& f) { halffacets_.push_back(f); }

  void toggle(int index) override { switches[index] = !switches[index]; }
...
private:
  ...
  {
    return OGL::Double_point(CGAL::to_double(p.x()), CGAL::to_double(p.y()), CGAL::to_double(p.z()));
  }

  static OGL::Double_segment double_segment(const Segment_3& s)
  {
    return OGL::Double_segment(double_point(s.source()), double_point(s.target()));
  }

  static void draw(Vertex_const_handle v, const Nef_polyhedron&, CGAL::OGL::Polyhedron& P)
  {
    Point_3 bp = v->point();
  ...
  static void draw(Halfedge_const_handle e, const Nef_polyhedron&, CGAL::OGL::Polyhedron& P)
  {
  ...
  static void draw(Halffacet_const_handle f, const Nef_polyhedron&, CGAL::OGL::Polyhedron& P)
  {
    if (f->incident_volume()->mark()) return;  // Skip halffaces facing solid volume
    OGL::DFacet g;
    Halffacet_cycle_const_iterator fc;  // all facet cycles:
    CGAL_forall_facet_cycles_of(fc, f) if (fc.is_shalfedge())
    {  // non-trivial facet cycle
      g.new_facet_cycle();
    ...
    g.set_normal(CGAL::to_double(v.x()), CGAL::to_double(v.y()), CGAL::to_double(v.z()), f->mark());
    P.push_back(g);
  }
  ...
  static void set_R(Bbox_3& bbox, const Nef_polyhedron& N)
  {
    ...
    bbox = Bbox_3(bbox.xmin() * 10, bbox.ymin() * 10, bbox.zmin() * 10, bbox.xmax() * 10,
                  bbox.ymax() * 10, bbox.zmax() * 10);
  }
