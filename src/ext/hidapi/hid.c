...
typedef CONFIGRET(__stdcall *CM_Get_DevNode_PropertyW_)(DEVINST dnDevInst, CONST DEVPROPKEY *PropertyKey,
...
static void hid_internal_get_ble_info(struct hid_device_info *dev, DEVINST dev_node)
{
  ...
  static DEVPROPKEY DEVPKEY_NAME = {
    {0xb725f130, 0x47ef, 0x101a, 0xa5, 0xf1, 0x02, 0x60, 0x8c, 0x9e, 0xeb, 0xac},
    10};  // DEVPROP_TYPE_STRING
  static DEVPROPKEY PKEY_DeviceInterface_Bluetooth_DeviceAddress = {
    {0x2BD67D8B, 0x8BEB, 0x48D5, 0x87, 0xE0, 0x6C, 0xDA, 0x34, 0x28, 0x04, 0x0A},
    1};  // DEVPROP_TYPE_STRING
  static DEVPROPKEY PKEY_DeviceInterface_Bluetooth_Manufacturer = {
    {0x2BD67D8B, 0x8BEB, 0x48D5, 0x87, 0xE0, 0x6C, 0xDA, 0x34, 0x28, 0x04, 0x0A},
    4};  // DEVPROP_TYPE_STRING

  /* Manufacturer String */
  len = 0;
  cr = CM_Get_DevNode_PropertyW(dev_node, &PKEY_DeviceInterface_Bluetooth_Manufacturer, &property_type,
                                NULL, &len, 0);
  if (cr == CR_BUFFER_SMALL && property_type == DEVPROP_TYPE_STRING) {
    free(dev->manufacturer_string);
    dev->manufacturer_string = (wchar_t *)calloc(len, sizeof(BYTE));
    CM_Get_DevNode_PropertyW(dev_node, &PKEY_DeviceInterface_Bluetooth_Manufacturer, &property_type,
                             (PBYTE)dev->manufacturer_string, &len, 0);
  }

  /* Serial Number String (MAC Address) */
  len = 0;
  cr = CM_Get_DevNode_PropertyW(dev_node, &PKEY_DeviceInterface_Bluetooth_DeviceAddress, &property_type,
                                NULL, &len, 0);
  if (cr == CR_BUFFER_SMALL && property_type == DEVPROP_TYPE_STRING) {
    free(dev->serial_number);
    dev->serial_number = (wchar_t *)calloc(len, sizeof(BYTE));
    CM_Get_DevNode_PropertyW(dev_node, &PKEY_DeviceInterface_Bluetooth_DeviceAddress, &property_type,
                             (PBYTE)dev->serial_number, &len, 0);
  }
  ...
  if (cr == CR_BUFFER_SMALL && property_type == DEVPROP_TYPE_STRING) {
    ...
    CM_Get_DevNode_PropertyW(dev_node, &DEVPKEY_NAME, &property_type, (PBYTE)dev->product_string, &len,
                             0);
  }
}
...
static void hid_internal_get_info(struct hid_device_info *dev)
{
  ...
  static DEVPROPKEY DEVPKEY_Device_CompatibleIds = {
    {0xa45c254e, 0xdf1c, 0x4efd, 0x80, 0x20, 0x67, 0xd1, 0x46, 0xa8, 0x50, 0xe0},
    4};  // DEVPROP_TYPE_STRING_LIST

  if (!CM_Get_Device_Interface_PropertyW || !CM_Locate_DevNodeW || !CM_Get_Parent ||
  ...
  if (mbsrtowcs(interface_path, &tmp, len, &state) == (size_t)-1) goto end;

  /* Get the device id from interface path */
  len = 0;
  cr = CM_Get_Device_Interface_PropertyW(interface_path, &DEVPKEY_Device_InstanceId, &property_type,
                                         NULL, &len, 0);
  if (cr == CR_BUFFER_SMALL && property_type == DEVPROP_TYPE_STRING) {
    device_id = (wchar_t *)calloc(len, sizeof(BYTE));
    cr = CM_Get_Device_Interface_PropertyW(interface_path, &DEVPKEY_Device_InstanceId, &property_type,
                                           (PBYTE)device_id, &len, 0);
  }
  ...
  if (cr == CR_BUFFER_SMALL && property_type == DEVPROP_TYPE_STRING_LIST) {
    ...
    cr = CM_Get_DevNode_PropertyW(dev_node, &DEVPKEY_Device_HardwareIds, &property_type,
                                  (PBYTE)hardware_ids, &len, 0);
  }
  if (cr != CR_SUCCESS) goto end;
  ...
  for (wchar_t *hardware_id = hardware_ids; *hardware_id; hardware_id += wcslen(hardware_id) + 1) {
    ...
    if (dev->interface_number != -1) break;
  }
  ...
  if (cr == CR_BUFFER_SMALL && property_type == DEVPROP_TYPE_STRING_LIST) {
    ...
    cr = CM_Get_DevNode_PropertyW(dev_node, &DEVPKEY_Device_CompatibleIds, &property_type,
                                  (PBYTE)compatible_ids, &len, 0);
  }
  if (cr != CR_SUCCESS) goto end;
                                                                  ...
                                                                  unsigned short product_id)
{
  ...
  device_info_set =
    SetupDiGetClassDevsA(&interface_class_guid, NULL, NULL, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);
  ...
  for (;;) {
    ...
    res = SetupDiEnumDeviceInterfaces(device_info_set, NULL, &interface_class_guid, device_index,
    ...
    res = SetupDiGetDeviceInterfaceDetailA(device_info_set, &device_interface_data, NULL, 0,
                                           &required_size, NULL);
    ...
    res = SetupDiGetDeviceInterfaceDetailA(device_info_set, &device_interface_data,
                                           device_interface_detail_data, required_size, NULL, NULL);
    ...
    if ((vendor_id == 0x0 || attrib.VendorID == vendor_id) &&
      ...
      struct hid_device_info *tmp =
        hid_get_device_info(device_interface_detail_data->DevicePath, read_handle);
...
HID_API_EXPORT hid_device *HID_API_CALL hid_open_path(const char *path)
{
  ...
  if (hid_init() < 0) goto end_of_function;
  ...
  if (device_handle == INVALID_HANDLE_VALUE) {
    ...
    if (device_handle == INVALID_HANDLE_VALUE) goto end_of_function;
  }

  /* Set the Input Report buffer size to 64 reports. */
  if (!HidD_SetNumInputBuffers(device_handle, 64)) goto end_of_function;

  /* Get the Input Report length for the device. */
  if (!HidD_GetPreparsedData(device_handle, &pp_data)) goto end_of_function;

  if (HidP_GetCaps(pp_data, &caps) != HIDP_STATUS_SUCCESS) goto end_of_function;
...
int HID_API_EXPORT HID_API_CALL hid_read_timeout(hid_device *dev, unsigned char *data, size_t length,
                                                 int milliseconds)
{
  DWORD bytes_read = 0;
  size_t copy_len = 0;
                                                        ...
                                                        size_t length)
{
  ...
  if (length >= dev->feature_report_length) {
    ...
    if (dev->feature_buf == NULL) dev->feature_buf = (unsigned char *)malloc(dev->feature_report_length);
    buf = dev->feature_buf;
    memcpy(buf, data, length);
...
int HID_API_EXPORT_CALL HID_API_CALL hid_get_manufacturer_string(hid_device *dev, wchar_t *string,
                                                                 size_t maxlen)
{
  if (!dev->device_info || !string || !maxlen) return -1;
