...
public:
  ...
  basic_memory_file(const char *pathname_) { open(pathname_); }
  ...
  void open(const char *pathname_)
  {
...
#ifdef _WIN32
    _fh = ::CreateFileA(pathname_, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
                        FILE_ATTRIBUTE_NORMAL, nullptr);
    _fmh = nullptr;

    if (_fh != INVALID_HANDLE_VALUE) {
      _fmh = ::CreateFileMapping(_fh, nullptr, PAGE_READONLY, 0, 0, nullptr);

      if (_fmh != nullptr) {
        _data = static_cast<char_type *>(::MapViewOfFile(_fmh, FILE_MAP_READ, 0, 0, 0));

        if (_data) {
          _size = ::GetFileSize(_fh, nullptr) / sizeof(char_type);
        }
      }
    }
#else
    ...
    if (_fh > -1) {
      ...
      if (::fstat(_fh, &sbuf_) > -1) {
        _data = static_cast<const char_type *>(::mmap(0, sbuf_.st_size, PROT_READ, MAP_SHARED, _fh, 0));
  ...
  const char_type *data() const { return _data; }
