...
public:
  ...
  using difference_type = typename std::iterator_traits<char_iterator>::difference_type;
  using pointer = typename std::iterator_traits<char_iterator>::pointer;
  using reference = typename std::iterator_traits<char_iterator>::reference;

  basic_utf8_in_iterator() : _it(char_iterator()), _end(char_iterator()), _char(0) {}

  explicit basic_utf8_in_iterator(const char_iterator& it_, const char_iterator& end_)
    : _it(it_), _end(it_), _char(0)
  {
    if (it_ != end_) {
      next();
    }
  ...
  char_type operator*() const { return _char; }

  bool operator==(const basic_utf8_in_iterator& rhs_) const { return _it == rhs_._it; }
  ...
  char_iterator get() const { return _it; }
...
private:
  ...
  char len(const char_iterator& it_) const
  {
    ...
    return ch_ < 0x80 ? 1 : ch_ >> 5 == 0x06 ? 2 : ch_ >> 4 == 0x0e ? 3 : ch_ >> 3 == 0x1e ? 4 : 1;
  }
};
...
public:
  ...
  basic_utf8_out_iterator() : _count(0), _index(0) {}

  explicit basic_utf8_out_iterator(const char_iterator& it_, const char_iterator& end_)
    : _it(it_), _count(0), _index(0)
  {
    if (it_ != end_) {
      next();
    }
  ...
  char operator*() const { return _bytes[_index]; }

  bool operator==(const basic_utf8_out_iterator& rhs_) const { return _it == rhs_._it; }
...
private:
  ...
  void next()
  {
    ...
    switch (_count) {
    case 1: _bytes[0] = static_cast<char>(ch_); break;
    case 2:
  ...
  {
    return ch_ < 0x80 ? 1 : ch_ < 0x800 ? 2 : ch_ < 0x10000 ? 3 : 4;
  }
};
...
public:
  ...
  basic_utf16_in_iterator() : _it(char_iterator()), _end(char_iterator()), _char(0) {}

  explicit basic_utf16_in_iterator(const char_iterator& it_, const char_iterator& end_)
    : _it(it_), _end(it_), _char(0)
  {
    if (it_ != end_) {
      next();
  ...
  bool operator==(const basic_utf16_in_iterator& rhs_) const { return _it == rhs_._it; }
...
public:
  ...
  basic_utf16_out_iterator() : _count(0), _index(0) {}

  explicit basic_utf16_out_iterator(const char_iterator& it_, const char_iterator& end_)
    : _it(it_), _count(0), _index(0)
  {
    if (it_ != end_) {
      next();
  ...
  out_char operator*() const { return _chars[_index]; }

  bool operator==(const basic_utf16_out_iterator& rhs_) const { return _it == rhs_._it; }
...
private:
  ...
  void next()
  {
    ...
    switch (_count) {
    case 1: _chars[0] = static_cast<out_char>(ch_); break;
    case 2:
      _chars[0] = static_cast<out_char>((ch_ >> 10) + 0xdc00u - (0x10000 >> 10));
      _chars[1] = static_cast<out_char>((ch_ & 0x3ff) + 0xdc00u);
      break;
    }
  }

  char len(const std::size_t ch_) const { return ch_ > 0xffff ? 2 : 1; }
};
...
public:
  using iterator_category = std::forward_iterator_tag;
  using value_type = typename std::iterator_traits<char_iterator>::value_type;
  using difference_type = typename std::iterator_traits<char_iterator>::difference_type;
  using pointer = typename std::iterator_traits<char_iterator>::pointer;
  using reference = typename std::iterator_traits<char_iterator>::reference;

  basic_flip_iterator() : _it(char_iterator()) {}

  explicit basic_flip_iterator(const char_iterator& it_) : _it(it_) {}
  ...
  bool operator==(const basic_flip_iterator& rhs_) const { return _it == rhs_._it; }
