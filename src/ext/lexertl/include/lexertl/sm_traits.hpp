...
struct basic_sm_traits {
  enum { char_24_bit = sizeof(ch_type) > 2, compressed = comp, lookup = look, is_dfa = dfa_nfa };
...
struct basic_sm_traits<ch_type, sm_type, true, look, dfa_nfa> {
  enum { char_24_bit = sizeof(ch_type) > 2, compressed = true, lookup = look, is_dfa = dfa_nfa };
  using input_char_type = ch_type;
