  ...
  static void charset(state_type& state_, string_token& token_)
  {
    ...
    do {
      if (ch_ == '\\') {
        ...
        if (chset_) {
          char_state temp_state_(str_ + 1, str_ + str_len_, state_._id, state_._flags, state_._locale,
                                 nullptr);
    ...
    for (; ptr_->from.first != 0; ++ptr_) {
      ...
      if (range_.first >= ptr_->from.first && range_.first <= ptr_->from.second) {
        if (ptr_->to.first <= ptr_->to.second) {
          ...
          out_.insert(range(
            first_, range_.second > ptr_->from.second
                      ? ptr_->to.second
...
private:
  ...
  static void posix(state_type& state_, string_token& token_)
  {
    ...
    if (state_.eos()) {
      ...
      case 'x':
        ...
        break;
      default: unknown_posix(state_); break;
      }
    }
  }
  ...
  static void alnum_alpha(state_type& state_, string_token& token_, const bool negate_)
  {
    ...
    if (type_ == unknown) {
      ...
      if (type_ == alnum) {
        // alnum
        str_ = sizeof(input_char_type) == 1 ? make_alnum(state_._locale)
                                            : std::string(R"([\p{Ll}\p{Lu}\p{Nd}])");
      } else {
        // alpha
        str_ =
          sizeof(input_char_type) == 1 ? make_alpha(state_._locale) : std::string(R"([\p{Ll}\p{Lu}])");
      }
  ...
  static void graph(state_type& state_, string_token& token_, const bool negate_)
  {
    ...
    if (*graph_) {
      ...
      const char *str_ = sizeof(input_char_type) == 1 ? "[\x21-\x7e]" : R"([^\p{Z}\p{C}])";
  ...
  static void lower(state_type& state_, string_token& token_, const bool negate_)
  {
    ...
    if (*lower_) {
      ...
      std::string str_ =
        sizeof(input_char_type) == 1 ? create_lower(state_._locale) : std::string("[\\p{Ll}]");
  ...
  static void print_punct(state_type& state_, string_token& token_, const bool negate_)
  {
    ...
    if (type_ == unknown) {
      ...
      if (type_ == print) {
        // print
        str_ = sizeof(input_char_type) == 1 ? "[\x20-\x7e]" : R"([\p{C}])";
      } else {
        // punct
        str_ =
          sizeof(input_char_type) == 1 ? R"([!"#$%&'()*+,\-./:;<=>?@[\\\]^_`{|}~])" : R"([\p{P}\p{S}])";
      }
  ...
  static void upper(state_type& state_, string_token& token_, const bool negate_)
  {
    ...
    if (*upper_) {
      ...
      std::string str_ =
        sizeof(input_char_type) == 1 ? create_upper(state_._locale) : std::string("[\\p{Lu}]");
  ...
  template <typename state_type>
  static void insert_charset(const char *str_, state_type& state_, string_token& token_,
                             const bool negate_)
  {
    // Some systems have strlen in namespace std.
    using namespace std;

    char_state temp_state_(str_ + 1, str_ + strlen(str_), state_._id, state_._flags, state_._locale,
                           nullptr);
    string_token temp_token_;
  ...
  static const char *charset_shortcut(state_type& state_, std::size_t& str_len_)
  {
    ...
    case 'D': str_ = "[^0-9]"; break;
    case 'p': str_ = unicode_escape(state_); break;
    case 's': str_ = "[ \t\n\r\f\v]"; break;
    case 'S': str_ = "[^ \t\n\r\f\v]"; break;
...
#include "table.inc"
    ...
    for (const block *entry_ = lookup_; entry_->_name; ++entry_) {
      ...
      for (; source_ != state_._curr && *name_; ++source_, ++name_) {
        if (*source_ != static_cast<typename state_type::char_type>(*name_)) break;
      }

      if (source_ == state_._curr && !*name_) return str_ = entry_->_func();
    }
...
#include "unicode.hpp"
  ...
  template <typename state_type>
  static void charset_range(const bool chset_, state_type& state_, bool& eos_,
                            typename state_type::char_type& ch_, const input_char_type prev_,
                            string_token& chars_)
  {
    if (chset_) {
      std::ostringstream ss_;
