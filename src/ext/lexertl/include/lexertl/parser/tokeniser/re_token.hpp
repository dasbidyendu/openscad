...
struct basic_re_token {
  ...
  basic_re_token(const token_type type_ = token_type::BEGIN) : _type(type_), _extra(), _str() {}
  ...
  char precedence(const token_type type_) const
  {
    ...
    static const char
      precedence_table_[static_cast<int>(token_type::END) + 1][static_cast<int>(token_type::END) + 1] = {
    ...
    return precedence_table_[static_cast<int>(_type)][static_cast<int>(type_)];
  }

  const char *precedence_string() const
  {
    // Moved in here for Solaris compiler.
    static const char *precedence_strings_[static_cast<int>(token_type::END) + 1] = {
