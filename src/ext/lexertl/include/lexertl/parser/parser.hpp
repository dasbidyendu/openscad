                           ...
                           const bool seen_bol_)
  {
    ...
    do {
      ...
      case '=':
        ...
        break;
      case '>': reduce(cr_id_, nl_id_); break;
      default:  {
    ...
    _node_ptr_vector.emplace_back(
      std::make_unique<end_node>(id_, user_id_, next_dfa_, push_dfa_, pop_dfa_));
...
private:
  ...
  void reduce(id_type& cr_id_, id_type& nl_id_)
  {
    ...
    case token_type::REGEX:
      ...
      break;
    case token_type::OREXP:      orexp(handle_); break;
    case token_type::SEQUENCE:   _token_stack.emplace(std::make_unique<token>(token_type::OREXP)); break;
    case token_type::SUB:        sub(handle_); break;
    case token_type::EXPRESSION: _token_stack.emplace(std::make_unique<token>(token_type::SUB)); break;
    case token_type::REPEAT:     repeat(handle_); break;
    case token_type::BOL:        bol(handle_); break;
    case token_type::EOL:        eol(handle_, cr_id_, nl_id_); break;
    case token_type::CHARSET:    charset(handle_, compressed()); break;
    case token_type::OPENPAREN:  openparen(handle_); break;
    case token_type::OPT:
    case token_type::AOPT:
      optional(rhs_->_type == token_type::OPT);
      _token_stack.emplace(std::make_unique<token>(token_type::DUP));
      break;
    ...
    case token_type::AREPEATN:
      repeatn(rhs_->_type == token_type::REPEATN, handle_.top().get());
      _token_stack.emplace(std::make_unique<token>(token_type::DUP));
      break;
    default: throw runtime_error("Internal error in regex_parser::reduce."); break;
    }
  }

  void orexp(token_stack& handle_)
  {
    assert(handle_.top()->_type == token_type::OREXP && (handle_.size() == 1 || handle_.size() == 3));

    if (handle_.size() == 1) {
      _token_stack.emplace(std::make_unique<token>(token_type::REGEX));
    } else {
      handle_.pop();
      ...
      perform_or();
      _token_stack.emplace(std::make_unique<token>(token_type::OREXP));
    }
  }
  ...
  {
    assert((handle_.top()->_type == token_type::SUB && handle_.size() == 1) || handle_.size() == 2);

    if (handle_.size() == 1) {
      _token_stack.emplace(std::make_unique<token>(token_type::SEQUENCE));
    } else {
      handle_.pop();
      assert(handle_.top()->_type == token_type::EXPRESSION);
      // perform join
      sequence();
      _token_stack.emplace(std::make_unique<token>(token_type::SUB));
    }
  }

  void repeat(token_stack& handle_)
  {
    assert(handle_.top()->_type == token_type::REPEAT && handle_.size() >= 1 && handle_.size() <= 3);

    if (handle_.size() == 1) {
      _token_stack.emplace(std::make_unique<token>(token_type::EXPRESSION));
    } else {
      handle_.pop();
      assert(handle_.top()->_type == token_type::DUP);
      _token_stack.emplace(std::make_unique<token>(token_type::REPEAT));
    }
  }
...
#endif
  {
    assert(handle_.top()->_type == token_type::BOL && handle_.size() == 1);
...
#endif
  {
    ...
    assert(handle_.top()->_type == token_type::EOL && handle_.size() == 1);
  ...
  {
    assert(handle_.top()->_type == token_type::CHARSET && handle_.size() == 1);
  ...
  void create_sequence(std::unique_ptr<token>& token_)
  {
    ...
    _token_stack.emplace(std::make_unique<token>(token_type::OPENPAREN));
    _token_stack.emplace(std::make_unique<token>(token_type::REGEX));
    _token_stack.emplace(std::make_unique<token>(token_type::CLOSEPAREN));
  }
  ...
  {
    typename string_token_vector::const_iterator iter_ = std::find_if(
      data_[0].begin(), data_[0].end(),
                    ...
                    string_token_vector data_[3])
  {
    ...
    do {
      iter_ = std::find_if(iter_, end_, [&token_](const std::unique_ptr<string_token>& rhs_) {
  ...
  {
    assert(handle_.top()->_type == token_type::OPENPAREN && handle_.size() == 3);
