...
void sm_to_csm(const sm& sm_, char_state_machine& csm_)
{
  ...
  using string_token_vector = typename char_state_machine::string_token_vector;
  ...
  for (id_type i_ = 0; i_ < dfas_; ++i_) {
    ...
    const std::size_t alphabet_ = internals_._dfa_alphabet[i_] - transitions_index;
    ...
    for (std::size_t c_ = 0; c_ < 256; ++c_, ++ptr_) {
      if (*ptr_ >= transitions_index) {
        string_token& token_ = token_vector_[*ptr_ - transitions_index];

        token_.insert(typename string_token::range(index_type(c_), index_type(c_)));
      }
    }
