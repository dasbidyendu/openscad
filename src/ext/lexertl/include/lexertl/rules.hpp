...
public:
  ...
  void push(const string& regex_, const id_type id_, const id_type user_id_ = npos())
  {
    ...
    if (_regexes.front().back()[_regexes.front().back().size() - 2]._type == detail::token_type::EOL) {
  ...
  // Add rule with no id
  void push(const rules_char_type *curr_dfa_, const rules_char_type *regex_,
            const rules_char_type *new_dfa_)
  {
    push(curr_dfa_, string(regex_), new_dfa_);
  }

  void push(const rules_char_type *curr_dfa_, const rules_char_type *regex_start_,
  ...
  void push(const rules_char_type *curr_dfa_, const rules_char_type *regex_start_,
            const rules_char_type *regex_end_, const id_type id_, const rules_char_type *new_dfa_,
            const id_type user_id_ = npos())
  {
...
private:
  ...
  void tokenise(const string& regex_, token_vector& tokens_, const id_type id_,
                const rules_char_type *name_)
  {
    ...
    do {
      ...
      if (rhs_._type != detail::token_type::DIFF && lhs_->precedence(rhs_._type) == ' ') {
        std::ostringstream ss_;
      ...
      if (rhs_._type == detail::token_type::MACRO) {
        ...
        if (iter_ == _macro_map.end()) {
          ...
          const bool bol_ = tokens_.size() == 1 && first_._type == detail::token_type::CHARSET &&
                            first_._str.size() == 1 &&
                            first_._str._ranges[0] == typename token::string_token::range('^', '^');
          const bool eol_ = state_._end == regex_.c_str() + regex_.size() &&
                            second_._type == detail::token_type::CHARSET && second_._str.size() == 1 &&
      ...
      if (diff_) {
        ...
        case '-':
          ...
          break;
        case '+': lhs_->_str.insert(rhs_._str); break;
        }
            ...
            const rules_char_type *new_dfa_, const bool check_, const id_type user_id_ = npos())
  {
    ...
    for (std::size_t i_ = 0, size_ = next_dfas_.size(); i_ < size_; ++i_) {
      ...
      if (_regexes[curr_].back()[_regexes[curr_].back().size() - 2]._type == detail::token_type::EOL) {
  ...
  void check_for_invalid_id(const id_type id_) const
  {
    ...
    if (id_ == npos()) {
      throw runtime_error(
        "The id npos is reserved for the "
        "UNKNOWN token.");
    }
  }
};
