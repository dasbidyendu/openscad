...
struct match_results {
  ...
  match_results(const iter_type& start_, const iter_type& end_, const bool bol_ = true,
                const id_type state_ = 0)
    : id(0), user_id(npos()), first(start_), second(start_), eoi(end_), bol(bol_), state(state_)
  {
  }
...
template <typename iter, typename id_type = uint16_t,
          std::size_t flags =
            bol_bit | eol_bit | skip_bit | again_bit | multi_state_bit | recursive_bit | advance_bit>
struct recursive_match_results : public match_results<iter, id_type, flags> {
  ...
  recursive_match_results(const iter& start_, const iter& end_, const bool bol_ = true,
                          const id_type state_ = 0)
    : match_results<iter, id_type, flags>(start_, end_, bol_, state_), stack()
  {
  }
