...
public:
  ...
  static void generate_cpp(const std::string& name_, const basic_state_machine<char_type, id_type>& sm_,
                           const bool pointers_, std::ostream& os_)
  {
    ...
    os_ << "        results_.id = " << static_cast<std::size_t>(internals_._eoi) << ";\n";
    ...
    output_char_loop(internals_._features, additional_tabs_, pointers_, os_,
                     std::integral_constant<bool, (sizeof(typename sm::traits::input_char_type) > 1)>());
    ...
    os_ << "        }\n";
    os_ << "    }\n\n";
    output_quit(os_, std::integral_constant<bool, (sizeof(typename sm::traits::input_char_type) > 1)>());
    ...
    if (internals_._features & again_bit) {
      // We want a number regardless of id_type.
      os_ << "\n        if (id_ == " << static_cast<std::size_t>(internals_._eoi);

      if (internals_._features & recursive_bit) {
        os_ << " || (pop_ && !results_.stack.empty() &&\n";
        // We want a number regardless of id_type.
        os_ << "            results_.stack.top().second == " << static_cast<std::size_t>(internals_._eoi)
            << ')';
      }
  ...
  template <typename char_type, typename id_type>
  static void dump_tables(const basic_state_machine<char_type, id_type>& sm_, const std::size_t tabs_,
                          const bool pointers_, std::ostream& os_)
  {
    const typename detail::basic_internals<id_type>& internals_ = sm_.data();
    ...
    // We want numbers regardless of id_type.
    os_ << "0x" << std::hex << static_cast<std::size_t>(internals_._dfa_alphabet[0]);

    for (std::size_t col_ = 1; col_ < dfas_; ++col_) {
      // We want numbers regardless of id_type.
      os_ << ", 0x" << std::hex << static_cast<std::size_t>(internals_._dfa_alphabet[col_]);
    }
    ...
    for (std::size_t dfa_ = 0; dfa_ < dfas_; ++dfa_) {
      ...
      const std::size_t rows_ = internals_._dfa[dfa_].size() / dfa_alphabet_;
      ...
      for (std::size_t row_ = 0; row_ < rows_; ++row_) {
        dump_row(row_ == 0, ptr_, dfa_name_, dfa_alphabet_, pointers_, os_);
...
protected:
  ...
  static void dump_row(const bool first_, const id_type *& ptr_, const std::string& dfa_name_,
                       const id_type dfa_alphabet_, const bool pointers_, std::ostream& os_)
  {
    if (pointers_) {
      bool zero_ = *ptr_ == 0;
      ...
      if (first_) {
        // We want numbers regardless of id_type.
        os_ << dfa_name_ << " + 0x" << std::hex << static_cast<std::size_t>(*ptr_++) * dfa_alphabet_;
      } else if (!zero_) {
      ...
      for (id_type id_index_ = id_index; id_index_ < transitions_index; ++id_index_, ++ptr_) {
        os_ << ", ";
        zero_ = *ptr_ == 0;
      ...
      for (id_type alphabet_ = transitions_index; alphabet_ < dfa_alphabet_; ++alphabet_, ++ptr_) {
        // We want numbers regardless of id_type.
        os_ << ", ";
      ...
      for (id_type alphabet_ = 1; alphabet_ < dfa_alphabet_; ++alphabet_, ++ptr_) {
        // We want numbers regardless of id_type.
        os_ << ", 0x" << std::hex << static_cast<std::size_t>(*ptr_);
      }
    }
  ...
  template <typename id_type>
  static void output_char_loop(const id_type features_, const std::size_t additional_tabs_,
                               const bool pointers_, std::ostream& os_, const std::false_type&)
  {
    output_tabs(additional_tabs_, os_);
  ...
  template <typename id_type>
  static void output_char_loop(const id_type features_, const std::size_t additional_tabs_,
                               const bool pointers_, std::ostream& os_, const std::true_type&)
  {
    output_tabs(additional_tabs_, os_);
  ...
  static void output_quit(std::ostream& os_, const std::true_type&) { os_ << "quit:\n"; }
};
}  // namespace lexertl
