...
protected:
  ...
  static string double_escape_char(const id_type ch_)
  {
    ...
    default: {
      if (ch_ < 32 || ch_ > 126) {
        out_ << '\\';
        out_ << 'x';
        out_ << std::hex << static_cast<std::size_t>(ch_);
      } else {
        out_ << char_type(ch_);
      }
  ...
  static void dump_ex(id_type dfa_id_, const typename char_state_machine::dfa& dfa_, rules& rules_,
                      ostream& stream_)
  {
    ...
    for (std::size_t i_ = 0; i_ < states_; ++i_) {
      ...
      if (i_ == 0) {
        stream_ << "    " << name << " [shape = doublecircle, xlabel=\"" << rules_.state(dfa_id_)
                << "\"];" << std::endl;
      } else if (state_._end_state) {
        stream_ << "    " << name
                << " [shape = doublecircle, xlabel=\"id =" << static_cast<std::size_t>(state_._id)
                << "\"];" << std::endl;
      } else {
    ...
    for (std::size_t i_ = 0; i_ < states_; ++i_) {
      ...
      for (; iter_ != end_; ++iter_) {
        ...
        for (; ranges_iter_ != ranges_end_; ++ranges_iter_) {
          if (ranges_iter_->first == '^' || ranges_iter_->first == ']') {
            stream_ << "\\\\";
          }
          ...
          if (ranges_iter_->first != ranges_iter_->second) {
            ...
            if (ranges_iter_->second == '^' || ranges_iter_->second == ']') {
              stream_ << "\\\\";
            }
      ...
      if (state_._end_state) {
        ...
        stream_ << "    " << src_name << " -> " << dst_name << " [style = \"dashed\"];" << std::endl;
      }
    }
  }
  ...
  static void trailer(ostream& stream_) { stream_ << "}" << std::endl; }

  static void open_bracket(ostream& stream_) { stream_ << "["; }

  static void negated(ostream& stream_) { stream_ << "^"; }

  static void close_bracket(ostream& stream_) { stream_ << "]"; }
};
