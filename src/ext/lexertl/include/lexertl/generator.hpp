...
template <typename rules, typename sm,
          typename char_traits = basic_char_traits<typename sm::traits::input_char_type>>
...
public:
  ...
  static void build(const rules& rules_, sm& sm_)
  {
    ...
    for (id_type index_ = 0; index_ < (id_type)size_; ++index_) {
      if (rules_.regexes()[index_].empty()) {
        ...
        // Regex syntax tree
        observer_ptr<node> root_ =
          build_tree(rules_, index_, node_ptr_vector_, charset_map_, cr_id_, nl_id_);

        build_dfa(charset_map_, root_, internals_, temp_sm_, index_, cr_id_, nl_id_);

        if (internals_._dfa[index_].size() / internals_._dfa_alphabet[index_] >= sm_traits::npos()) {
          // Overflow
          throw runtime_error(
            "The data type you have chosen "
  ...
  static observer_ptr<node> build_tree(const rules& rules_, const std::size_t dfa_,
                                       ...
                                       id_type& cr_id_, id_type& nl_id_)
  {
    ...
    root_ = parser_.parse(*regex_iter_, *id_iter_, *user_id_iter_, *next_dfa_iter_, *push_dfa_iter_,
                          *pop_dfa_iter_, rules_.flags(), cr_id_, nl_id_, seen_bol_);
    ++regex_iter_;
    ++id_iter_;
    ++user_id_iter_;
    ++next_dfa_iter_;
    ...
    while (regex_iter_ != regex_iter_end_) {
      observer_ptr<node> rhs_ = parser_.parse(*regex_iter_, *id_iter_, *user_id_iter_, *next_dfa_iter_,
                                              *push_dfa_iter_, *pop_dfa_iter_, rules_.flags(), cr_id_,
                                              nl_id_, (rules_.features()[dfa_] & bol_bit) != 0);
...
protected:
  ...
  static void build_dfa(const charset_map& charset_map_, const observer_ptr<node> root_,
                        internals& internals_, sm& sm_, const id_type dfa_index_, id_type& cr_id_,
                        id_type& nl_id_)
  {
    ...
    if (cr_id_ != sm_traits::npos() || nl_id_ != sm_traits::npos()) {
      ...
      zero_id_ = sm_traits::compressed
                   ? *set_mapping_[charset_map_.find(string_token(0, 0))->second].begin()
                   : sm_traits::npos();
    }
    ...
    if (dfa_alphabet_ > sm_traits::npos()) {
      // Overflow
      throw runtime_error(
        "The data type you have chosen cannot hold "
        "the dfa alphabet.");
    }
    ...
    closure(followpos_, seen_sets_, seen_vectors_, hash_vector_, static_cast<id_type>(dfa_alphabet_),
            dfa_);
    ...
    for (id_type index_ = 0; index_ < static_cast<id_type>(seen_vectors_.size()); ++index_) {
      ...
      // Intersect charsets
      build_equiv_list(*seen_vectors_[index_], set_mapping_, equiv_list_, is_dfa());

      for (auto& equivset_ : equiv_list_) {
        const id_type transition_ = closure(equivset_->_followpos, seen_sets_, seen_vectors_,
                                            hash_vector_, static_cast<id_type>(dfa_alphabet_), dfa_);

        if (transition_ != sm_traits::npos()) {
          observer_ptr<id_type> ptr_ =
            &dfa_.front() + ((static_cast<std::size_t>(index_) + 1) * dfa_alphabet_);
    ...
    fix_clashes(eol_set_, cr_id_, nl_id_, zero_id_, dfa_, dfa_alphabet_, compressed());
  ...
  // Uncompressed
  static void fix_clashes(const id_type_set& eol_set_, const id_type cr_id_, const id_type nl_id_,
                          const id_type /*zero_id_*/, typename internals::id_type_vector& dfa_,
  ...
  static void append_dfa(const charset_list&, const internals&, sm&, const id_type,
                         const std::true_type&)
  {
  ...
  // state_machine version
  static void create(internals& internals_, sm& sm_, const id_type_vector& features_,
                     const std::true_type&)
  {
  ...
  static void build_set_mapping(const charset_list& charset_list_, internals& internals_,
                                const id_type dfa_index_, index_set_vector& set_mapping_)
  {
  ...
  // char_state_machine version
  static void fill_lookup(const string_token&, observer_ptr<id_type_vector>, const id_type,
  ...
  // state_machine version
  static void fill_lookup(const string_token& charset_, observer_ptr<id_type_vector> lookup_,
                          const id_type index_, const std::true_type&)
  {
  ...
  static id_type closure(const node_vector& followpos_, node_set_vector& seen_sets_,
                         node_vector_vector& seen_vectors_, size_t_vector& hash_vector_,
                         const id_type size_, id_type_vector& dfa_)
  {
    ...
    for (observer_ptr<node> node_ : followpos_) {
      closure_ex(node_, end_state_, id_, user_id_, next_dfa_, push_dfa_, pop_dfa_, *set_ptr_,
                 *vector_ptr_, hash_);
    }
  ...
  // NFA version
  static void build_equiv_list(const node_vector& vector_, const index_set_vector& set_mapping_,
                               equivset_list& lhs_, const std::false_type&)
  {
  ...
  // DFA version
  static void build_equiv_list(const node_vector& vector_, const index_set_vector& set_mapping_,
                               equivset_list& lhs_, const std::true_type&)
  {
    equivset_list rhs_;
  ...
  static void fill_rhs_list(const node_vector& vector_, const index_set_vector& set_mapping_,
                            equivset_list& list_)
  {
      ...
      if (!node_->end_state()) {
        ...
        if (token_ != node::null_token()) {
          if (token_ == parser::bol_token() || token_ == parser::eol_token()) {
            ...
            list_.emplace_back(std::make_unique<equivset>(set_mapping_[token_], token_, node_->greedy(),
                                                          node_->followpos()));
          }
        }
      }
