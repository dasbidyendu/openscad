...
struct basic_char_state_machine {
  ...
  static_assert(std::is_move_assignable<dfa>::value && std::is_move_constructible<dfa>::value,
                "dfa is not movable.");
  ...
  void append(const string_token_vector& token_vector_, const internals& internals_,
              const id_type dfa_index_)
  {
      ...
      for (id_type col_index_ = 0; col_index_ < alphabet_; ++col_index_, ++ptr_) {
        ...
        if (next_ > 0) {
          ...
          if (trans_iter_ == state_._transitions.end()) {
            trans_iter_ = state_._transitions
                            .insert(id_type_string_token_pair(static_cast<id_type>(next_ - 1),
                                                              token_vector_[col_index_]))
                            .first;
          } else {
