...
public:
  ...
  basic_stream_shared_iterator() : _master(false), _live(false), _index(shared::npos()), _shared(nullptr)
  {
  }

  basic_stream_shared_iterator(istream& stream_, const std::size_t buff_size_ = 1024,
  ...
  basic_stream_shared_iterator& operator=(const basic_stream_shared_iterator& rhs_)
  {
    if (this != &rhs_) {
      _master = false;
      _index = rhs_._master ? rhs_._shared->lowest() : rhs_._index;
  ...
  bool operator==(const basic_stream_shared_iterator& rhs_) const
  {
    return _index == rhs_._index &&
           (_shared == rhs_._shared ||
            (_index == shared::npos() || rhs_._index == shared::npos()) && (!_shared || !rhs_._shared));
  }

  bool operator!=(const basic_stream_shared_iterator& rhs_) const { return !(*this == rhs_); }
...
private:
  ...
  public:
    ...
    shared(istream& stream_, const std::size_t buff_size_, const std::size_t increment_)
      : _ref_count(0), _increment(increment_), _stream(stream_)
    {
    ...
    bool reload_buffer()
    {
      ...
      if (lowest_ == 0) {
        ...
        memcpy(&_buffer.front(), &_buffer[lowest_], start_ * sizeof(char_type));
    ...
    void erase(basic_stream_shared_iterator *ptr_)
    {
      ...
      if (iter_ != _clients.end()) _clients.erase(iter_);
    }
