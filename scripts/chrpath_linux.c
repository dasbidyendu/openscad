...
int chrpath(const char *filename, const char *newpath, int convert)
{
  ...
  if (NULL == newpath && 0 == convert) oflags = O_RDONLY;
  else oflags = O_RDWR;
  ...
  if (lseek(fd, phdr.p_offset, SEEK_SET) == -1 || read(fd, dyns, phdr.p_filesz) != (int)phdr.p_filesz) {
  ...
  rpathoff = -1;
  for (rpath_dyns_index = 0; dyns[rpath_dyns_index].d_tag != DT_NULL; ++rpath_dyns_index) {
  ...
  for (i = 0; i < ehdr.e_shnum; ++i) {
    ...
    if (shdr.sh_type == SHT_STRTAB) break;
  }
  if (i == ehdr.e_shnum) {
  ...
  printf("%s: %s=%s\n", filename, elf_tagname(dyns[rpath_dyns_index].d_tag), rpath);
   ...
   */
  for (i = rpathoff + rpathlen; (i < (int)shdr.sh_size && strtab[i] == '\0'); i++);
  i--;

  if (i > (int)(rpathoff + rpathlen)) rpathlen = i - rpathoff;
...
int elf_open(const char *filename, int flags, Elf_Ehdr *ehdr)
{
  ...
  if (0 != memcmp(ehdr->e_ident, ELFMAG, SELFMAG) || ehdr->e_ident[EI_CLASS] != ELFCLASS ||
      ehdr->e_ident[EI_DATA] != ELFDATA2 || ehdr->e_ident[EI_VERSION] != EV_CURRENT) {
    fprintf(stderr,
#ifdef WORDS_BIGENDIAN
...
int elf_find_dynamic_section(int fd, Elf_Ehdr *ehdr, Elf_Phdr *phdr)
{
  ...
  for (i = 0; i < ehdr->e_phnum; ++i) {
    ...
    if (phdr->p_type == PT_DYNAMIC) break;
  }
  if (i == ehdr->e_phnum) {
...
{
  switch (tag) {
  case DT_RPATH: return "RPATH"; break;
#if defined(DT_RUNPATH)
  case DT_RUNPATH: return "RUNPATH"; break;
#endif /* DT_RUNPATH */
  }
  return "UNKNOWN";
}
...
int killrpath(const char *filename)
{
  ...
  for (i = 0; dyns[i].d_tag != DT_NULL; ++i) {
    dyns[dynpos] = dyns[i];
    if (!elf_dynpath_tag(dyns[i].d_tag)) dynpos++;
  }
  for (; dynpos < i; ++dynpos) dyns[dynpos].d_tag = DT_NULL;

  if (lseek(fd, phdr.p_offset, SEEK_SET) == -1 || write(fd, dyns, phdr.p_filesz) != (int)phdr.p_filesz) {
...
static struct option long_options[] = {
  {"convert", 0, 0, 'c'}, {"delete", 0, 0, 'd'},  {"help", 0, 0, 'h'},   {"keepgoing", 0, 0, 'k'},
  {"list", 0, 0, 'l'},    {"replace", 1, 0, 'r'}, {"version", 0, 0, 'v'}};
...
int main(int argc, char *const argv[])
{
  ...
  do {
    ...
    case 'r': newpath = optarg; break;
    case 'v':
      printf("%s version %s\n", PACKAGE, VERSION);
      exit(0);
      break;
    case 'l': /* This is the default action */ newpath = NULL; break;
    case -1:  break;
  ...
  while (optind < argc && (!retval || keep_going)) {
    if (remove) retval |= killrpath(argv[optind++]);
    else /* list by default, replace if path is set */
