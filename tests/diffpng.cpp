...
public:
  ...
  uint8_t Get_Red(unsigned int i) const { return (Data[i % Data.size()] & 0xFF); }
  uint8_t Get_Green(unsigned int i) const { return ((Data[i % Data.size()] >> 8) & 0xFF); }
  uint8_t Get_Blue(unsigned int i) const { return ((Data[i % Data.size()] >> 16) & 0xFF); }
  uint8_t Get_Alpha(unsigned int i) const { return ((Data[i % Data.size()] >> 24) & 0xFF); }
  void Set(uint8_t r, uint8_t g, uint8_t b, uint8_t a, unsigned int i)
  {
    Data[i] = r | (g << 8) | (b << 16) | (a << 24);
  }
  unsigned int Get_Width() const { return Width; }
  unsigned int Get_Height() const { return Height; }
  void Set(unsigned int x, unsigned int y, unsigned int d) { Data[x + y * Width] = d; }
  unsigned int Get(unsigned int x, unsigned int y) const { return Data[x + y * Width]; }
  unsigned int Get(unsigned int i) const { return Data[i % Data.size()]; }
  const string& Get_Name() const { return Name; }
  unsigned int *Get_Data() { return &Data[0]; }
  const unsigned int *Get_Data() const { return &Data[0]; }
...
static bool option_matches(const char *arg, const string& option_name)
{
  ...
  return (string_arg == "--" + option_name) or (string_arg == "-" + option_name);
}
...
public:
    ...
    for (int i = 1; i < argc; ++i) {
      ...
      if (option_matches(argv[i], "fov")) {
        ...
        if (++i < argc) {
          ...
          if (temporary < 0) {
            cout << " invalid_argument(" << "-threshold must be positive";
          }
        ...
        cerr << "Warningoption/file \"" << argv[i] << "\" ignored\n";
      }
      //			}
  ...
  void Print_Args() const
  {
    cout << "Field of view is " << FieldOfView << " degrees\n"
         ...
         << "The Gamma is " << Gamma << "\n"
         << "The Display's luminance is " << Luminance << " candela per meter squared\n"
...
static vector<float> Copy(const float *img, const unsigned int width, const unsigned int height)
{
...
struct White {
  White() { AdobeRGBToXYZ(1.f, 1.f, 1.f, x, y, z); }
...
bool Yee_Compare_Engine(CompareArgs& args)
{
  ...
  const float num_one_degree_pixels = 2.f * tan(args.FieldOfView * 0.5 * M_PI / 180) * 180 / M_PI;
...
bool LevelClimberCompare(CompareArgs& args)
{
  ...
  if (test == false) {
    ...
    test = Yee_Compare_Engine(args);
    cout << "Result: " << test << " ErrorStr:" << args.ErrorStr << "\n";
    if (args.ImgDiff) args.ImgDiff->WriteToFile(args.ImgDiff->Get_Name() + ".diff.sampleddown.png");
    if (test) return true;
  };
